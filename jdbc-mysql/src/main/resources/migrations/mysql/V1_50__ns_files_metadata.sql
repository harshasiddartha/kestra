CREATE TABLE IF NOT EXISTS namespace_file_metadata (
    ...
    `directory` BOOL GENERATED ALWAYS AS (value ->> '$.directory' = 'true') STORED NOT NULL,
...
CREATE INDEX ix_directory_last_deleted_tenant_namespace_path ON `namespace_file_metadata` (`directory`, `last`, `deleted`, `tenant_id`, `namespace`, `path`);
